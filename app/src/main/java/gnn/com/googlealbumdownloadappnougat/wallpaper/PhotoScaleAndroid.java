...
public class PhotoScaleAndroid {
    ...
    static Matrix getMatrix(Bitmap bitmap, int displayWidth, int displayHeight) {
        ...
        scale = getScale(displayWidth, displayHeight, originalWidth, originalHeight);
    ...
    static float getScale(int displayWidth, int displayHeight, int originalWidth, int originalHeight) {
        float scale;
        // width / height < 1 = width < height
        // ratio1 < ratio2 = width1 < width2
        if (((float) originalWidth) / originalHeight < ((float) displayWidth) / displayHeight) {
